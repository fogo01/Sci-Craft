...
public class ItemSciCraftArmor extends ItemArmor {
    ...
    public String getArmorTexture(ItemStack itemStack, Entity entity, int slot, String type) {
        if (((ItemSciCraftArmor)itemStack.getItem()).getArmorMaterial() == Materials.Armor.ARMOR_UTILITY)
            return Reference.MOD_ID + ":textures/models/armor/Utility_layer_" + (((ItemArmor)itemStack.getItem()).armorType == 2 ? "2" : "1") + ".png";
        for (int i = 0; i < Names.Items.ARMOR_TYPES.length; i++) {
            if (itemStack.getUnlocalizedName().contains(Names.Items.ARMOR_TYPES[i]))
                return Reference.MOD_ID + ":textures/models/armor/" + Names.Items.ARMOR_TYPES[i] + "_layer_" + (((ItemArmor)itemStack.getItem()).armorType == 2 ? "2" : "1") + ".png";
        }
        return null;
    }
