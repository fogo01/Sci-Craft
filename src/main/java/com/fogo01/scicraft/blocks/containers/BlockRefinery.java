...
public class BlockRefinery extends BlockSciCraftContainer {
    ...
    public BlockRefinery(boolean blockState) {
        super();
        this.setBlockName(Names.Blocks.REFINERY);
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float p_149727_7_, float p_149727_8_, float p_149727_9_) {
        if (!player.isSneaking())
            if (player.getHeldItem() != null && (player.getHeldItem().getItem() == ModItems.OIL_BUCKET || player.getHeldItem().getItem() == Items.bucket)){
                if (world.getTileEntity(x, y, z) instanceof TileEntityRefinery)
                    ((TileEntityRefinery)world.getTileEntity(x, y, z)).moveLiquid(player.getHeldItem().getItem() == ModItems.OIL_BUCKET ? ModFluids.OIL : ModFluids.FUEL, player);
                return false;
            }
        ...
        if (!world.isRemote)
            if (world.getTileEntity(x, y, z) instanceof TileEntityRefinery)
                player.openGui(SciCraft.instance, GUIs.REFINERY.ordinal(), world, x, y, z);
    ...
    public static void updateBlockState(boolean crushing, World world, int x, int y, int z) {
        ...
        if (crushing) {
            world.setBlock(x, y, z, ModBlocks.REFINERY_ACTIVE);
        } else {
            world.setBlock(x, y, z, ModBlocks.REFINERY);
        }
