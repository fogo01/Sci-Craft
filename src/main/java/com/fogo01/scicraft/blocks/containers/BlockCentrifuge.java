...
public class BlockCentrifuge extends BlockSciCraftContainer {
    ...
    public BlockCentrifuge(boolean blockState) {
        super();
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float p_149727_7_, float p_149727_8_, float p_149727_9_) {
        ...
        if (!world.isRemote)
            if (world.getTileEntity(x, y, z) instanceof TileEntityCentrifuge)
                player.openGui(SciCraft.instance, GUIs.CENTRIFUGE.ordinal(), world, x, y, z);
    ...
    public static void updateBlockState(boolean smelting, World world, int x, int y, int z) {
        ...
        if (smelting) {
            world.setBlock(x, y, z, ModBlocks.CENTRIFUGE_ACTIVE);
        } else {
            world.setBlock(x, y, z, ModBlocks.CENTRIFUGE);
        }
