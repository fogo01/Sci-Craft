...
public class BlockBatteryCell extends BlockSciCraftContainer {
    ...
    public BlockBatteryCell() {
        super();
        this.setBlockName(Names.Blocks.BATTERY_CELL);
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float p_149727_7_, float p_149727_8_, float p_149727_9_) {
        if (!player.isSneaking())
            if (player.getHeldItem() != null && (player.getHeldItem().getItem() == ModItems.WRENCH || player.getHeldItem().getItem() == ModItems.MULTIMETER))
                return false;
        if (!world.isRemote)
            if (world.getTileEntity(x, y, z) instanceof TileEntityBatteryCell)
                player.openGui(SciCraft.instance, GUIs.BATTERY_CELL.ordinal(), world, x, y, z);
    ...
    public TileEntity createNewTileEntity(World p_149915_1_, int p_149915_2_) {
        return new TileEntityBatteryCell();
    }
