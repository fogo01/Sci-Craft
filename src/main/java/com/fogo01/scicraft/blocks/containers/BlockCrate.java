...
import com.fogo01.scicraft.blocks.BlockConnectedTexture;
...
public class BlockCrate extends BlockSciCraftContainer {
    ...
    public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side) {
        return BlockConnectedTexture.getIcon(world, x, y, z, side, icons);
        /*
        IIcon icon = icons[0];
        ...
        for (int y1 = -1; y1 <= 1; y1++) {
            for (int z1 = -1; z1 <= 1; z1++) {
                for (int x1 = -1; x1 <= 1; x1++) {
                    canConnect[count] = CanConnectTo(world, x + x1, y + y1, z + z1);
                    blocking[count] = isBlockSolid(world, x + x1, y + y1, z + z1, side == 0 ? 1 : side == 1 ? 0 : side == 2 ? 3 : side == 3 ? 2 : side == 4 ? 5 : 4);
